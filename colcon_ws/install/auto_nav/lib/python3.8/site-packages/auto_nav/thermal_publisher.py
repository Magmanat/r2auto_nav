...
while (time.time()-t0)<1: # wait 1sec for sensor to start
    try:
        ...
        sensor = AMG8833(addr=0x69) # start AMG8833
    except:
        sensor = AMG8833(addr=0x68)
    finally:
        pass
...
class Thermal(Node):
    ...
    def __init__(self):
        ...
        #create publisher for thermal cam array data
    ...
    def timer_callback(self):
        status,pixels = sensor.read_temp(pix_to_read) #retrieve data from cam
        #status is a Bool for presence of error
        thermal_msg=Float64MultiArray()
        thermal_msg.data = pixels
        self.publisher_.publish(thermal_msg)
        self.get_logger().info('Publishing Thermal Array')
